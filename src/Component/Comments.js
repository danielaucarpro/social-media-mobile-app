...
export default function Comments(props) {
    ...
    return (
        <>
            ...
            md:block md:mb-4
            sm:text-gray-400 sm:text-sm sm:mt-2 sm:hidden`}
                onClick={() => showMore(data.id)}>{data.expanded === false ? "See more comments" : "Hide comments"}</p>

            <p onClick={commentOnOff} className='lg:hidden md:hidden text-gray-400 cursor-pointer mt-3 text-sm'>{showComments ? 'Hide' : 'Show comments...'}</p>
            ...
            {data.cm.slice(0, data.itemsToShow).map((comment, index) => {
                ...
                md:block
                sm:text-sm sm:mt-2 sm:mb-2 ${showComments ? '' : 'sm:hidden'}`}
                    key={index}>
